...
def square_root_rational(x,abs_tol=Rational(1,1000)):
    '''Returns square root of x to absolute precision'''
    ...
    z = float(x)
    y = Rational(0,1)
    estimate = Rational(0,1)
    while abs(float(estimate)**2 - abs(z)) > float(abs_tol) and estimate <= x:
        estimate = (Rational(x.num, x.denom)+ Rational(y.num, y.denom))/2
        if float(estimate)**2 < z:
            y = estimate
        else:
            x = estimate
    return estimate
